...
namespace Neowise {
	...
	FMatrix3::FMatrix3(real x0, real y0, real z0, real x1, real y1, real z1, real x2, real y2, real z2) 
		: columns{
			{ x0, y0, z0 },
			{ x1, y1, z1 },
			{ x2, y2, z2 }
	...
	FMatrix3& FMatrix3::operator+=(real r) {
		columns[0] += r;
		columns[1] += r;
		columns[2] += r;
		return *this;
	...
	FMatrix3::operator FQuaternion() const {
		...
		const auto biggestVal = sqrt(fourBiggestSquaredMinus1 + (real)(1)) * (real)(0.5);
		const auto mult = (real)(0.25) / biggestVal;
	...
	FMatrix3 FMatrix3::operator+(real r) const {
	...
	FVector3& FMatrix3::operator[](int32 i) {
		return columns[i];
	}

	FVector3 FMatrix3::operator[](int32 i) const {
		return columns[i];
	}
