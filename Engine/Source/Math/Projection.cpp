...
namespace Neowise {
	...
	void CProjection::setNearZ(real value) {
		_zNear = value;
		_dirty = true;
	}

	void CProjection::setFarZ(real value) {
		_zFar = value;
		_dirty = true;
	}
	...
	void CProjection::setRectSize(real left, real right, real top, real bottom) {
		_left = left;
		_right = right;
		_top = top;
		_bottom = bottom;
	...
	void CProjection::setFovY(real value) {
		_fovY = value;
		_dirty = true;
	}
	...
	void CProjection::setupPerspective(const SProjectionPerspectiveCreateInfo& createInfo) {
		...
		const auto range = tan(createInfo.fovY / (real)(2)) * createInfo.nearPlaneZ;
		...
		if (right - left != 0) {
			_mat[0][0] = (real(2) * createInfo.nearPlaneZ) / (right - left);
		}
		_mat[1][1] = (real(2) * createInfo.nearPlaneZ) / (top - bottom);
		_mat[2][2] = real(-0.9999988);
		_mat[2][3] = real(-1);
		_mat[3][2] = real(-1.9999988) * createInfo.nearPlaneZ;
	}
	...
	void CProjection::setupOrthographic(const SProjectionOrthographicCreateInfo& createInfo) {
		if (createInfo.right - createInfo.left != 0) {
			_mat[0][0] = (real)(2) / (createInfo.right - createInfo.left);
		}
		if (createInfo.top - createInfo.bottom != 0) {
			_mat[1][1] = (real)(2) / (createInfo.top - createInfo.bottom);
		}
		_mat[2][2] = -(real)(2) / (createInfo.farPlaneZ - createInfo.nearPlaneZ);
