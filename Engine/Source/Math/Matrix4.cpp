...
namespace Neowise {
	...
	FMatrix4& FMatrix4::operator+=(real r) {
		columns[0] += r;
		columns[1] += r;
		columns[2] += r;
		columns[3] += r;
	...
	FMatrix4 FMatrix4::operator+(real r) const {
	...
	FVector4& FMatrix4::operator[](int32 i) {
		return columns[i];
	}

	FVector4 FMatrix4::operator[](int32 i) const {
		return columns[i];
	}
